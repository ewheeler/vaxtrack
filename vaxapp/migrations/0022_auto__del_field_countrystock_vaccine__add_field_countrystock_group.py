...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Deleting field 'CountryStock.vaccine'
        db.delete_column('vaxapp_countrystock', 'vaccine_id')

        # Adding field 'CountryStock.group'
        db.add_column('vaxapp_countrystock', 'group', self.gf('django.db.models.fields.related.ForeignKey')(default=0, to=orm['vaxapp.VaccineGroup']), keep_default=False)

        # Adding M2M table for field products on 'CountryStock'
        db.create_table('vaxapp_countrystock_products', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('countrystock', models.ForeignKey(orm['vaxapp.countrystock'], null=False)),
            ('vaccine', models.ForeignKey(orm['vaxapp.vaccine'], null=False))
        ))
        db.create_unique('vaxapp_countrystock_products', ['countrystock_id', 'vaccine_id'])


    def backwards(self, orm):
        
        # We cannot add back in field 'CountryStock.vaccine'
        raise RuntimeError(
            "Cannot reverse this migration. 'CountryStock.vaccine' and its values cannot be restored.")

        # Deleting field 'CountryStock.group'
        db.delete_column('vaxapp_countrystock', 'group_id')

        # Removing M2M table for field products on 'CountryStock'
        db.delete_table('vaxapp_countrystock_products')
